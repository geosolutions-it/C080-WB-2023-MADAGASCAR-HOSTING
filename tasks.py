...
def update(ctx):
    ...
    try:
        current_allowed = ast.literal_eval(os.getenv('ALLOWED_HOSTS') or \
                                           "['{public_fqdn}', '{public_host}', 'localhost', 'django', 'mad_geonode3',]".format(**envs))
    except ValueError:
        current_allowed = []
...
--settings={0}".format(_localsettings()), pty=True)
    ctx.run("python manage.py loaddata /usr/src/mad_geonode3/fixtures/initial_data.json \
--settings={0}".format(_localsettings()), pty=True)
...
def _localsettings():
    settings = os.getenv('DJANGO_SETTINGS_MODULE', 'mad_geonode3.settings')
    return settings
